...
def greedy_PDF(
        ...
        encoder_output, encoder_hidden, class_embeds, src, global_mask, output_size, size_input, input_obj_id, fix_root):
    """
...
#     ys = encoder_output.new_full([batch_size, 1, 128], bos_index, dtype=torch.long)
    ...
    if size_input:
        ys_0[:,0,[2,3]] = output_size[:,0,:]
    ys_1 = encoder_output
    ys_wh_gmm = encoder_output.new_full([batch_size, 1, gmm_comp_num*6], 0., dtype=torch.float)
    ys_xy_gmm = encoder_output.new_full([batch_size, 1, gmm_comp_num*6], 0., dtype=torch.float)
    ...
    for step in range(max_output_length-1):
        ...
        with torch.no_grad():
            ...
            if size_input:
                sample_wh = output_size[:,:step+2,:]
                #if wh_gmm is not None:
                    #wh_gmm = output_size
            if fix_root:
                sample_wh[:,:,0][input_obj_id[:,:step+2] == 5] = 1.0
                sample_wh[:,:,1][input_obj_id[:,:step+2] == 5] = 1.0
                sample_xy[:,:,0][input_obj_id[:,:step+2] == 5] = 0.5
                sample_xy[:,:,1][input_obj_id[:,:step+2] == 5] = 0.5
