...
class Sentence_Embeddings(nn.Module):
    ...
    def __init__(self, 
        ...
        self.parent_id_embeddings = nn.Embedding(obj_classes_size, hidden_size, padding_idx=0)
    ...
    def forward(self, input_token, input_obj_id, input_parent_id, segment_label, token_type):

        inputs_embeds = self.word_embeddings(input_token)
        
        input_id_embdes = self.obj_id_embeddings(input_obj_id)
        if input_parent_id != None:
            input_parent_embeds = self.parent_id_embeddings(input_parent_id)
        segment_embeddings = self.sentence_type(segment_label)
        token_type_embeddings = self.token_type(token_type)
        if input_parent_id != None:
            embeddings = inputs_embeds + segment_embeddings + input_id_embdes + token_type_embeddings + input_parent_embeds
        else:
